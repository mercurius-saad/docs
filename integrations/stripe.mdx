...
The full charge → capture → receipt → payout flow is documented in [Booking & Payments](/workflows/booking-and-payments). Key `StripeService` methods:
...
See [Booking & Payments — Step 3](/workflows/booking-and-payments#step-3-make-payment) for the full payment flow with booking state transitions.
